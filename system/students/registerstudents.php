    ...
    <div class="col-12">
        ...
        <div class="card">
            ...
            <div class="card-body table-responsive p-0">
                <?php
                $db= dbConn();
                $sql="SELECT s.Id as Id,s.SRegNo as SRegNo,s.FirstName,s.LastName,g.Name as GradeId,g.Id AS gId,s.MobileNo,GROUP_CONCAT(DISTINCT "
                        . "d.ClassName ORDER BY d.ClassName ASC SEPARATOR ', ') AS ClassName "
                        . "FROM students s INNER JOIN registerstudents r ON r.StudentId=s.Id "
                        . "LEFT JOIN classdetails d ON d.Id=r.ClassId "
                        . "LEFT JOIN grades g ON g.Id=s.GradeId "
                        . "LEFT JOIN approvalpayments a ON a.StudentId = s.Id "
                        . "WHERE s.status = 1 GROUP BY a.StudentId";
//                $sql = "SELECT s.Id as Id,s.SRegNo as SRegNo,s.FirstName,s.LastName,s.MobileNo,g.Name as GradeId, g.Id as gId, "
//                        . "GROUP_CONCAT(DISTINCT a.ClassName ORDER BY a.ClassName ASC SEPARATOR ', ') AS ClassName "
//                        . "FROM students s INNER JOIN grades g ON g.Id=s.GradeId "
